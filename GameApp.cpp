...
void GameApp::UpdateScene(float dt)
{
	...
	else if (m_KeyboardTracker.IsKeyPressed(Keyboard::A))
	{
	...
	else if (m_KeyboardTracker.IsKeyPressed(Keyboard::D))
	{
		m_PSConstantBuffer.dirLight[0] = DirectionalLight();
		m_PSConstantBuffer.pointLight[0] = PointLight();
	...
	{
		// 播放木箱动画
		m_CurrMode = ShowMode::Texture;
		m_pd3dImmediateContext->IASetInputLayout(m_pVertexLayout3D.Get());
		auto meshData = Geometry::CreateBox();
		ResetMesh(meshData);
		m_pd3dImmediateContext->VSSetShader(m_pVertexShader3D.Get(), nullptr, 0);
		m_pd3dImmediateContext->PSSetShader(m_pPixelShader3D.Get(), nullptr, 0);
		m_pd3dImmediateContext->PSSetShaderResources(0, 1, m_pTexture1.GetAddressOf());
		m_pd3dImmediateContext->PSSetShaderResources(0, 1, m_pTexture2.GetAddressOf());
		m_pd3dImmediateContext->PSSetShaderResources(0, 1, m_pTexture3.GetAddressOf());
		m_pd3dImmediateContext->PSSetShaderResources(0, 1, m_pTexture4.GetAddressOf());
		m_pd3dImmediateContext->PSSetShaderResources(0, 1, m_pTexture5.GetAddressOf());
		m_pd3dImmediateContext->PSSetShaderResources(0, 1, m_pTexture6.GetAddressOf());
	}

	if (m_CurrMode == ShowMode::WoodCrate)
	{
		static float phi = 0.0f, theta = 0.0f, z = 0.0f;
		phi += 0.0001f, theta += 0.00015f;
		z += 2 * dt;
		XMMATRIX W = XMMatrixRotationX(phi)* XMMatrixRotationY(theta);
		m_VSConstantBuffer.world = XMMatrixTranspose(W);
		m_VSConstantBuffer.worldInvTranspose = XMMatrixTranspose(InverseTranspose(W));
		/*m_pd3dImmediateContext->PSSetShaderResources(0, 1, m_pFlare.GetAddress0f());
		m_pd3dImmediateContext->PSSetShaderResources(1, 1, m_pFlarealpha.GetAddress0f());*/

		//让纹理动起来
		m_VSConstantBuffer.Tex = XMMatrixRotationZ(z);
	...
	else if (m_CurrMode == ShowMode::Texture) 
	{
		static float phi = 0.0f, theta = 0.0f, z = 0.0f;
		phi += 0.0001f, theta += 0.00015f;
		XMMATRIX W = XMMatrixRotationX(phi) * XMMatrixRotationY(theta);
...
void GameApp::DrawScene()
{
	...
	// 绘制几何模型
	//m_pd3dImmediateContext->PSSetShaderResources(0, 1, m_pWoodCrate.GetAddressOf());
	if (m_CurrMode == ShowMode::Texture)
	{
		for (int i = 0; i < m_IndexCount; i += 3) {
			if (i == 0)m_pd3dImmediateContext->PSSetShaderResources(0, 1, m_pTexture1.GetAddressOf());
			else if (i == 6)m_pd3dImmediateContext->PSSetShaderResources(0, 1, m_pTexture2.GetAddressOf());
			else if (i == 12)m_pd3dImmediateContext->PSSetShaderResources(0, 1, m_pTexture3.GetAddressOf());
			else if (i == 18)m_pd3dImmediateContext->PSSetShaderResources(0, 1, m_pTexture4.GetAddressOf());
			else if (i == 24)m_pd3dImmediateContext->PSSetShaderResources(0, 1, m_pTexture5.GetAddressOf());
			else if (i == 30)m_pd3dImmediateContext->PSSetShaderResources(0, 1, m_pTexture6.GetAddressOf());
...
bool GameApp::InitResource()
{
	...
	m_DirLight.diffuse = XMFLOAT4(1.6f, 1.6f, 1.6f, 1.0f);
	...
	m_SpotLight.diffuse = XMFLOAT4(2.0f, 2.0f, 2.0f, 1.0f);
	m_SpotLight.specular = XMFLOAT4(1.0f, 1.0f, 1.0f, 1.0f);
	m_SpotLight.att = XMFLOAT3(1.0f, 0.0f, 0.0f);
	m_SpotLight.spot = 12.0f;
	m_SpotLight.range = 20000.0f;
	...
	// 初始化木箱纹理
	HR(CreateDDSTextureFromFile(m_pd3dDevice.Get(), L"..\\纹理映射与采样器状态\\Texture\\Texture1.dds", nullptr, m_pTexture1.GetAddressOf()));
	HR(CreateDDSTextureFromFile(m_pd3dDevice.Get(), L"..\\纹理映射与采样器状态\\Texture\\Texture2.dds", nullptr, m_pTexture2.GetAddressOf()));
	HR(CreateDDSTextureFromFile(m_pd3dDevice.Get(), L"..\\纹理映射与采样器状态\\Texture\\Texture3.dds", nullptr, m_pTexture3.GetAddressOf()));
	HR(CreateDDSTextureFromFile(m_pd3dDevice.Get(), L"..\\纹理映射与采样器状态\\Texture\\Texture4.dds", nullptr, m_pTexture4.GetAddressOf()));
	HR(CreateDDSTextureFromFile(m_pd3dDevice.Get(), L"..\\纹理映射与采样器状态\\Texture\\Texture5.dds", nullptr, m_pTexture5.GetAddressOf()));
	HR(CreateDDSTextureFromFile(m_pd3dDevice.Get(), L"..\\纹理映射与采样器状态\\Texture\\Texture6.dds", nullptr, m_pTexture6.GetAddressOf()));
	HR(CreateDDSTextureFromFile(m_pd3dDevice.Get(), L"..\\纹理映射与采样器状态\\Texture\\flare+alpha.dds", nullptr, m_pWoodCrate.GetAddressOf()));
	// 初始化火焰纹理
	WCHAR strFile[40];
	...
	sampDesc.AddressW = D3D11_TEXTURE_ADDRESS_WRAP; //D3D11_TEXTURE_ADDRESS_WRAP
